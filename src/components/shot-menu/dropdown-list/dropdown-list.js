...
const DropdownList = () => {
  ...
  const { shotsList, setShotsList, setOpenShotSettingMenu, openShotSettingMenu } = useContext(ShotMenuContext)
  const dropdownMenu = useRef(document.querySelector('.dropdown-list-wrapper'))

  useEffect(
    () => {
      if(openShotSettingMenu['position'] !== null) {
        dropdownMenu.current.style.transform = 'translateY(0px)';
        }

        if(openShotSettingMenu['position'] === null) {
          dropdownMenu.current.style.transform = 'translateY(500px)';          
          }
    },
  ...
  const closeMenu = (arrayCopy) => {
    if (arrayCopy[openShotSettingMenu['position']]['aperture'] !== null
